import { d as h, L as g, j as m, s as n, x as p, h as u } from "./iframe.js";
import { n as f } from "./ref.js";
import { i as v } from "./keyed.js";
import { g as x } from "./dom.js";
...
class C extends g {
  constructor() {
    super(...arguments), this.messages = $(), this.mutationObserver = b("mutation", () => this.setUpTips()), this.closed = !1, this.calciteTipManagerClose = m({ cancelable: !1 });
  }
  static {
  ...
  // #region Rendering
  renderPagination() {
    const e = x(this.el), { selectedIndex: t, tips: a, total: c, messages: s } = this, r = s.next, l = s.previous, d = s.defaultPaginationLabel;
    return a.length > 1 ? p`<footer class=${n(i.pagination)}><calcite-action class=${n(i.pagePrevious)} .icon=${e === "ltr" ? o.chevronLeft : o.chevronRight} @click=${this.previousClicked} scale=m .text=${l}></calcite-action><span class=${n(i.pagePosition)}>${`${d} ${t + 1}/${c}`}</span><calcite-action class=${n(i.pageNext)} .icon=${e === "ltr" ? o.chevronRight : o.chevronLeft} @click=${this.nextClicked} scale=m .text=${r}></calcite-action></footer>` : null;
  }
  render() {
    const { closed: e, direction: t, headingLevel: a, groupTitle: c, selectedIndex: s, messages: r, total: l } = this, d = r.close;
    return l === 0 ? null : p`<section .ariaHidden=${e} class=${n(i.container)} .hidden=${e} @keydown=${this.tipManagerKeyDownHandler} tabindex=0 ${f(this.storeContainerRef)}><header class=${n(i.header)}>${T({ class: i.heading, level: a, children: c })}<calcite-action class=${n(i.close)} @click=${this.hideTipManager} scale=m .text=${d}><calcite-icon .icon=${o.close} scale=m></calcite-icon></calcite-action></header>${v(s, p`<div class=${n({
      [i.tipContainer]: !0,
