import { d as E, L as I, j as u, z as F, s as d, E as M, x as o, B as P, h as k } from "./iframe.js";
...
class L extends I {
  ...
  async load() {
    this.readCellContentsToAT = /safari/i.test(F()), this.listenOn(this.el.shadowRoot, "slotchange", this.handleSlotChange);
  }
  willUpdate(e) {
  ...
  calciteInternalTableRowFocusEvent(e) {
    const i = e.detail.cellPosition, s = e.detail.rowPosition, l = e.detail.destination, c = e.detail.lastCell, t = this.bodyRows?.filter((h) => !g(h)), a = this.allRows?.filter((h) => !g(h)), f = this.headRows[this.headRows.length - 1]?.positionAll, S = t[0]?.positionAll, w = t[t.length - 1]?.positionAll, R = this.footRows[0]?.positionAll, C = a[a.length - 1]?.positionAll, v = l === "next" && s === f, z = l === "previous" && s === R, T = l === "previous" && s === S, x = l === "next" && s === w;
    let r;
    switch (l) {
      case "first":
        r = 0;
  ...
  renderSelectionArea() {
    ...
    return o`<div class=${d(n.selectionArea)}><calcite-chip .kind=${this.selectedCount > 0 ? "brand" : "neutral"} .label=${l} .scale=${this.scale} .value=${l}>${l}</calcite-chip>${e > 0 && o`<calcite-chip icon=hide-empty .label=${c} .scale=${this.scale} title=${c ?? M} .value=${c}>${i}</calcite-chip>` || ""}${this.selectedCount > 0 && o`<calcite-button icon-start=x kind=neutral @click=${this.handleDeselectAllRows} round .scale=${this.scale} .title=${`${this.messages.clear} ${l} ${this.messages.row}`}>${this.messages.clear}</calcite-button>` || ""}<div class=${d(n.selectionActions)}><slot name=${m.selectionActions}></slot></div></div>`;
  }
  ...
  renderTHead() {
    return o`<thead><slot name=${m.tableHeader} ${p(this.tableHeadSlotEl)}></slot></thead>`;
  }
  renderTBody() {
    return o`<tbody><slot ${p(this.tableBodySlotEl)}></slot></tbody>`;
  }
  renderTFoot() {
    return o`<tfoot><slot name=${m.tableFooter} ${p(this.tableFootSlotEl)}></slot></tfoot>`;
  }
  render() {
