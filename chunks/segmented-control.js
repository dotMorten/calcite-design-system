...
class z extends p {
  ...
  async setFocus() {
    await E(this), (this.selectedItem || this.items[0])?.focus();
  }
  connectedCallback() {
  ...
  disconnectedCallback() {
    super.disconnectedCallback(), M(this), k(this);
  }
  // #endregion
  ...
  // #region Rendering
  render() {
    return this.el.role = "radiogroup", c`<div aria-errormessage=${d.validationMessage} .ariaInvalid=${this.status === "invalid"} class=${v(U.itemWrapper)}>${A({ disabled: this.disabled, children: c`<slot @slotchange=${this.handleDefaultSlotChange}></slot>${x({ component: this })}` })}</div>${this.validationMessage && this.status === "invalid" ? L({ icon: this.validationIcon, id: d.validationMessage, message: this.validationMessage, scale: this.scale, status: this.status }) : null}`;
  }
}
w("calcite-segmented-control", z);
export {
