import { d as S, L as O, s as b, q as $, x as w, c as p, h as Y } from "./iframe.js";
...
function A({ data: e, min: s, max: a, t: n }) {
  if (e.length === 0)
    return "";
...
const B = S`:host{display:block;block-size:100%}.svg{fill:currentColor;stroke:transparent;margin:0;display:block;block-size:100%;inline-size:100%;padding:0}.svg .graph-path--highlight{fill:var(--calcite-graph-highlight-fill-color, var(--calcite-color-brand));opacity:.5}:host([hidden]){display:none}[hidden]{display:none}`, k = {
  svg: "svg",
...
class E extends O {
  ...
  render() {
    ...
    const { min: g, max: d } = q(s);
    let y = g, f = d;
    (o < g[0] || o > g[0]) && (y = [o, 0]), (r > d[0] || r < d[0]) && (f = [r, d[1]]);
    const x = H({ min: y, max: f, width: l, height: t }), [M] = x([c, f[1]]), [m] = x([h, f[1]]), u = A({ data: s, min: g, max: d, t: x }), v = a ? `url(#linear-gradient-${i})` : void 0;
