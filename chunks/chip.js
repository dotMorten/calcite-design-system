import { d as h, L as d, j as a, i as v, s as i, x as t, h as g } from "./iframe.js";
import { e as o, n as r } from "./ref.js";
import { s as m } from "./dom.js";
...
class C extends d {
  ...
  renderCloseButton() {
    return t`<button .ariaLabel=${this.messages.dismissLabel} class=${i(c.close)} @click=${this.close} @keydown=${this.closeButtonKeyDownHandler} .tabIndex=${this.disabled ? -1 : 0} ${r(this.closeButtonEl)}><calcite-icon .icon=${l.close} .scale=${n(this.scale)}></calcite-icon></button>`;
  }
  renderIcon() {
  ...
  render() {
    ...
    return x({ disabled: e, children: t`<div .ariaChecked=${this.selectionMode !== "none" && this.interactive ? this.selected : void 0} .ariaLabel=${this.label} class=${i({
      [c.container]: !0,
    ...
    })} @click=${this.handleEmittingEvent} .role=${p} .tabIndex=${s ? -1 : 0} ${r(this.containerEl)}>${this.selectionMode !== "none" && this.renderSelectionIcon() || ""}${this.renderChipImage()}${this.icon && this.renderIcon() || ""}<span class=${i(c.title)}><slot @slotchange=${this.handleDefaultSlotChange}></slot></span>${this.closable && this.renderCloseButton() || ""}</div>` });
  }
}
g("calcite-chip", C);
export {
