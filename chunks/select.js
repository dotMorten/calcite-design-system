import { d, L as h, j as p, k as m, s as c, x as o, h as u } from "./iframe.js";
...
class A extends h {
  ...
  // #region Lifecycle
  constructor() {
    super(), this.componentToNativeEl = /* @__PURE__ */ new Map(), this.mutationObserver = k("mutation", () => this.populateInternalSelect()), this.disabled = !1, this.required = !1, this.scale = "m", this.status = "idle", this.validity = {
      valid: !1,
      badInput: !1,
      customError: !1,
      patternMismatch: !1,
  ...
  render() {
    ...
    return C({ disabled: e, children: o`<div class=${c(n.wrapper)}><select aria-errormessage=${r.validationMessage} .ariaInvalid=${this.status === "invalid"} .ariaLabel=${N(this)} class=${c(n.select)} .disabled=${e} @change=${this.handleInternalSelectChange} ${v(this.storeSelectRef)}><slot></slot></select>${this.renderChevron()}${w({ component: this })}</div>${this.validationMessage && this.status === "invalid" ? O({ icon: this.validationIcon, id: r.validationMessage, message: this.validationMessage, scale: this.scale, status: this.status }) : null}` });
  }
}
u("calcite-select", A);
export {
