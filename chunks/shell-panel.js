import { d as N, L as K, j as S, s as y, x as b, n as B, C as g, h as D } from "./iframe.js";
...
class R extends K {
  ...
  // #region Rendering
  renderHeader() {
    return z("header", b`<div class=${y(u.contentHeader)} .hidden=${!this.hasHeader}><slot name=${$.header} @slotchange=${this.handleHeaderSlotChange}></slot></div>`);
  }
  render() {
    const { collapsed: e, position: t, initialContentWidth: i, initialContentHeight: n, contentWidth: x, contentWidthMax: a, contentWidthMin: w, contentHeight: d, contentHeightMax: l, contentHeightMin: s, resizable: m, layout: h, displayMode: o } = this, c = M(this.el), C = o !== "float-content" && o !== "float" && m, k = C ? h === "horizontal" ? d ? { height: `${d}px` } : null : x ? { width: `${x}px` } : null : null, r = !e && C ? z("separator", b`<div .ariaLabel=${this.messages.resize} .ariaOrientation=${h === "horizontal" ? "vertical" : "horizontal"} .ariaValueMax=${h == "horizontal" ? l : a} .ariaValueMin=${h == "horizontal" ? s : w} .ariaValueNow=${h == "horizontal" ? d ?? n : x ?? i} class=${y(u.separator)} @keydown=${this.separatorKeyDown} role=separator tabindex=0 touch-action=none ${E(this.connectSeparator)}></div>`) : null, p = () => h === "horizontal" ? t === "start" ? g.calciteAnimateInDown : g.calciteAnimateInUp : c === "ltr" && t === "end" || c === "rtl" && t === "start" ? g.calciteAnimateInLeft : g.calciteAnimateInRight, v = z("content", b`<div class=${y({
      [g.rtl]: c === "rtl",
      [u.content]: !0,
    ...
    })} .hidden=${e} style=${B(k)} ${E(this.storeContentEl)}>${this.renderHeader()}<div class=${y(u.contentBody)}><slot></slot></div>${r}</div>`), H = [z("action-bar", b`<slot name=${$.actionBar} @slotchange=${this.handleActionBarSlotChange}></slot>`), v];
    return t === "end" && H.reverse(), b`<div class=${y({ [u.container]: !0, [u.floatAll]: o === "float-all" })}>${H}</div>`;
  }
}
