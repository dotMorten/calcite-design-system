import { p as i } from "./placeholder-image.js";
import { h as a } from "./formatting.js";
import { A as r } from "./resources14.js";
...
`, t = [{
  ...
  name: "tn"
}], l = t.map((e) => a`<calcite-avatar user-id="${e.id}" username="${e.name}"></calcite-avatar>`).join(""), m = t.map((e) => a`<calcite-avatar
        dir="rtl"
        class="calcite-mode-dark"
        user-id="${e.id}"
        username="${e.name}"
      ></calcite-avatar>`).join(""), s = () => a`
  <div>
    <style>
      .container {
        display: flex;
...
const p = ["simple", "missingThumbnail", "contrast"];
export {
  p as __namedExportsOrder,
  s as contrast,
  f as default,
  n as missingThumbnail,
  c as simple
};
