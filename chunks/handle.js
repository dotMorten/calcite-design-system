import { d, L as h, j as s, s as u, E as c, x as b, h as p } from "./iframe.js";
...
class S extends h {
  constructor() {
    super(...arguments), this.handleButton = g(), this.blurUnselectDisabled = !1, this.disabled = !1, this.messages = x({ blocking: !0 }), this.selected = !1, this.calciteHandleChange = s({ cancelable: !1 }), this.calciteHandleNudge = s({ cancelable: !1 }), this.calciteInternalAssistiveTextChange = s({ cancelable: !1 });
  }
  static {
  ...
  // #region Rendering
  render() {
    return y({ disabled: this.disabled, children: b`<span .ariaChecked=${this.disabled ? null : this.selected} .ariaDisabled=${this.disabled ? this.disabled : null} .ariaLabel=${this.disabled ? null : this.getAriaText("label")} class=${u({ [r.handle]: !0, [r.handleSelected]: !this.disabled && this.selected })} @blur=${this.handleBlur} @keydown=${this.handleKeyDown} role=radio tabindex=${(this.disabled ? null : 0) ?? c} title=${this.getTooltip() ?? c} ${f(this.handleButton)}><calcite-icon .icon=${H.drag} scale=s></calcite-icon></span>` });
  }
}
p("calcite-handle", S);
export {
