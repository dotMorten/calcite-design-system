import { d, L as h, s as i, E as n, x as o, h as p } from "./iframe.js";
...
class v extends h {
  ...
  render() {
    ...
    })} colSpan=${this.colSpan ?? n} @blur=${this.onContainerBlur} @focus=${this.onContainerFocus} .role=${this.parentRowType === "head" ? "columnheader" : "rowheader"} rowSpan=${this.rowSpan ?? n} scope=${t ?? n} .tabIndex=${this.selectionCell ? 0 : a ? -1 : 0} ${m(this.containerEl)}>${this.heading && o`<div class=${i(e.heading)}>${this.heading}</div>` || ""}${this.description && o`<div class=${i(e.description)}>${this.description}</div>` || ""}${this.selectionCell && this.selectionMode === "multiple" && o`<calcite-icon class=${i({ [e.active]: s || l })} .icon=${r} .scale=${g(this.scale)}></calcite-icon>` || ""}${(this.selectionCell || this.numberCell) && o`<span .ariaLive=${this.focused ? "polite" : "off"} class=${i(e.assistiveText)}>${this.screenReaderText}</span>` || ""}</th>`;
  }
}
