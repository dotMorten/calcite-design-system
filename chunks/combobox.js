...
import { c as oe, a as ne, d as ce, s as re, H as he } from "./form.js";
...
class ke extends j {
  ...
  constructor() {
    super(), this.filterItems = (() => {
      ...
      return $e((t, i = !1, s = !0) => {
        const l = Z([...this.data, ...this.groupData], t, this.effectiveFilterProps), a = this.getItemsAndGroups(), o = t === "";
        a.forEach((n) => {
          if (o) {
            n.itemHidden = !1;
            return;
          ...
          (e(p, l) || e(I, l)) && (n.itemHidden = !1), h || n.ancestors.forEach((u) => u.itemHidden = !1);
  ...
  connectedCallback() {
    super.connectedCallback(), me(this), oe(this), this.internalValueChangeFlag = !0, this.value = this.getValue(), this.internalValueChangeFlag = !1, this.mutationObserver?.observe(this.el, { childList: !0, subtree: !0 }), this.setFilteredPlacements(), C(this);
  }
  willUpdate(e) {
  ...
  loaded() {
    ne(this, this.getValue()), C(this), this.updateItems(), this.filterItems(this.filterText, !1, !1);
  }
  ...
  async refreshSelectionDisplay() {
    if (this.componentOnReady(), x(this.selectionMode) || !this.textInput.value)
      return;
    const { allSelectedIndicatorChipEl: e, chipContainerEl: t, selectionDisplay: i, placeholder: s, selectedIndicatorChipEl: l, textInput: { value: a } } = this, o = parseInt(getComputedStyle(t).gap.replace("px", "")), n = v(t), { fontSize: h, fontFamily: p } = getComputedStyle(a), u = (ee(s, `${h} ${p}`) || parseInt(Q)) + o, w = v(e), D = v(l), G = Math.max(w, D);
    if (this.setCompactSelectionDisplay({
      ...
      const A = Array.from(this.el.shadowRoot.querySelectorAll("calcite-chip")).filter((_) => _.closable), W = Math.round(n - ((this.selectedHiddenChipsCount > 0 ? D : 0) + o + u + o));
      this.refreshChipDisplay({ availableHorizontalChipElSpace: W, chipContainerElGap: o, chipEls: A }), this.setVisibleAndHiddenChips(A);
    }
  }
  setFloatingEl(e) {
  ...
  renderSelectedIndicatorChip() {
    ...
    if (e)
      ...
      const p = i().length;
      this.isAllSelected() ? n = !0 : p > 0 ? n = !1 : n = !0, h = `${p} ${this.messages.selected}`;
    } else t === "fit" && (n = !!(this.isAllSelected() && a === 0 || l === 0), h = a > 0 ? `+${l}` : `${l} ${this.messages.selected}`);
    return d`<calcite-chip class=${r({
      chip: !0,
      [c.chipInvisible]: n
    })} .label=${h} .scale=${s} title=${h ?? m} value ${g(o)}>${h}</calcite-chip>`;
  }
  ...
  renderInput() {
    const { guid: e, disabled: t, placeholder: i, selectionMode: s, selectedItems: l, open: a } = this, o = x(s), n = l[0], h = !a && o && !!n && !this.filterText;
    return d`<span class=${r({
      ...
      label: !0,
      "label--icon": !!n?.icon
    })}>${$(n)}</span>`) || ""}${f("input", d`<input aria-activedescendant=${this.activeDescendant ?? m} aria-controls=${`${y}${e}`} aria-errormessage=${P.validationMessage} aria-owns=${`${y}${e}`} aria-autocomplete=list .ariaExpanded=${a} aria-haspopup=listbox .ariaInvalid=${this.status === "invalid"} .ariaLabel=${be(this)} class=${r({
      [c.input]: !0,
      "input--single": !0,
      [c.inputHidden]: h,
  ...
  renderSelectedOrPlaceholderIcon() {
    ...
    return this.showingInlineIcon && f("selected-placeholder-icon", d`<span class="icon-start"><calcite-icon class=${r({
      [c.selectedIcon]: !o,
      [c.placeholderIcon]: o
    })} .flipRtl=${o ? i : l.iconFlipRtl} .icon=${o ? t : a} .scale=${S(this.scale)}></calcite-icon></span>`) || "";
  }
  renderChevronIcon() {
    const { open: e } = this;
  ...
  render() {
    const { selectionDisplay: e, guid: t, label: i, open: s, readOnly: l } = this, a = x(this.selectionMode), o = e === "all", n = e === "single", h = !a && e === "fit", p = !this.clearDisabled && this.value?.length > 0;
